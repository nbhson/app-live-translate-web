// SidePanel - minimal UI: Recording audio... + animation, no EN->VI (web handles translate)
...
const idleState=document.getElementById('idleState'), recordingIndicator=document.getElementById('recordingIndicator'), liveText=document.getElementById('liveText'), permissionOverlay=document.getElementById('permissionOverlay'), grantPermissionBtn=document.getElementById('grantPermissionBtn');
...
function startTabCapture(){
  showStatus('Getting tab stream (incl. iframe)...');
  chrome.runtime.sendMessage({ type: 'get-tab-stream-id' }, async (response)=>{
    if(!response || response.error){ showStatus('Tab capture failed, fallback to mic'); audioSourceSelect.value='mic'; startListening(); return; }
    try{
    ...
    }catch(err){ console.error(err); showStatus('Tab audio error, using mic'); audioSourceSelect.value='mic'; cleanupTabCapture(); startListening(); }
  });
}
...
function stopListening(){ isListening=false; if(recognition) try{recognition.stop();}catch{} if(silenceTimer){clearTimeout(silenceTimer); silenceTimer=null;} cleanupTabCapture(); updateUI(false); showStatus('Stopped'); }

function initRecognition(){
  const SR=window.SpeechRecognition||window.webkitSpeechRecognition;
  if(!SR){ showStatus('SpeechRecognition not supported'); return; }
  recognition=new SR(); recognition.continuous=true; recognition.interimResults=true; recognition.lang='en-US';
  recognition.onstart=()=>{ isListening=true; updateUI(true); showStatus(activeAudioTrack?'Capturing Tab audio (incl. iframe)...':'Listening mic...'); };
...
async function finalizeText(text){
  ...
  liveText.innerText=clean;
  broadcastToRoom(clean);
  showStatus(activeAudioTrack?'Tab audio → sent to app':'Mic → sent to app');
}
async function forceFinalize(text,rawLen){ if(silenceTimer){clearTimeout(silenceTimer); silenceTimer=null;} finalizedOffset=rawLen; await finalizeText(text); }
function updateUI(active){
  if(active){
    toggleBtn.className='btn btn-danger'; playIcon.style.display='none'; stopIcon.style.display='block'; btnText.innerText='Stop';
    idleState.classList.add('hidden'); recordingIndicator.classList.remove('hidden');
  } else {
    toggleBtn.className='btn btn-primary'; playIcon.style.display='block'; stopIcon.style.display='none'; btnText.innerText='Start';
    idleState.classList.remove('hidden'); recordingIndicator.classList.add('hidden'); liveText.innerText='';
  }
}
function clearContent(){ liveText.innerText=''; showStatus('Cleared'); }
